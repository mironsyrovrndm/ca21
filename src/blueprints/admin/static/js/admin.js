const setupRepeatableSection = (section) => {
  ...
  const createItem = () => {
    ...
    let source = template;
    if (template && template.tagName === "TEMPLATE") source = template.content.firstElementChild;
    if (!source) source = list.querySelector("[data-repeatable-item]");
...
document.addEventListener("submit", (event) => {
  ...
  if (form) sessionStorage.setItem(adminScrollKey, String(window.scrollY));
});
...
window.addEventListener("load", () => {
  ...
  if (stored) {
    ...
    if (!Number.isNaN(position)) window.scrollTo({ top: position });
